...
from sample.util.config_parser import ConfigParser_with_eval
...
def load_config(filename):
    """
    :param filename: ハイパーパラメータを記述した Ini形式っぽいテキストファイル.
    :return:
    """
    ...
    for keyname, subjson in json_obj.items():
        ...
        if type_of_subjson == dict:
            ...
            if type_of_subjson == np.ndarray:
                if subjson.dtype in [np.int8, np.int16, np.int32, np.int64, np.uint8, np.uint16, np.uint32,
                                     np.uint64]:
...
def hypparams():
    ...
    # TODO: コマンドラインでsampleのコンフィグを指定したいならコメントを外す
    #   要は中身のstrを渡しているだけだが、ここでは 手動でpathをしていしたいので全てコメントアウト
    ...
    # args = parser.parse_args()  # ここで /home/path_to_npbdaa/tests/test_pyhlm.py::test_hlm が渡されてしまう...
    # TODO: 上に同じく、コマンドラインでsampleのコンフィグを指定したいならコメントを外す
    ...
    pretrain_iter = section["pretrain_iter"]  # str?
    ...
    # TODO: で、結局 config_parser[f"{i + 1}_th"] の中身はなに？
    ...
    return letter_hsmm_hypparams, obs_hypparams, dur_hypparams, word_num, len_hypparams, hlm_hypparams, superstate_config, pretrain_iter, thread_num, train_iter, observation_dim


def test_hlm(hypparams):
    letter_hsmm_hypparams, obs_hypparams, dur_hypparams, word_num, len_hypparams, hlm_hypparams, superstate_config, pretrain_iter, thread_num, train_iter, observation_dim = hypparams
    thread_num = 1

    # %% make instance of distributions model
    ...
    # FIXME: ここで cython の import でエラーになる。
    # # or
    # for name, data in zip(files, datas):
