import os
import numpy as np
from pyhlm.prosodic_model import WeakLimitProsodicHDPHLM
...
def load_prosody_datas():
    pdatas = []
    names = np.loadtxt("files.txt", dtype=str)
    for name in names:
        tmp = np.loadtxt("DATA/" + name + ".f0").reshape((-1, 1))
        pdatas.append(np.concatenate((tmp, tmp), axis=1))
    return pdatas
...
model = WeakLimitProsodicHDPHLM(**hlm_hypparams, letter_hsmm=letter_hsmm, prosody_distns=prosody_obs_distns, dur_distns=dur_distns, length_distn=length_distn)

#%%
files = np.loadtxt("files.txt", dtype=str)
datas = load_datas()
pdatas = load_prosody_datas()

#%% Pre training.
for data, pdata in zip(datas, pdatas):
...
#%%
print("Add datas...")
for name, data, pdata in zip(files, datas, pdatas):
    model.add_data(data=data, prosody_data=pdata, **superstate_config[name], generate=False)
